    ...
    if (this.service.isAdminLoggedIn()) {
      ...
      this.http.post('http://localhost:8080/admin/addLibrarian', registerForm.value).subscribe(response => {
        ...
        if (response) {
          ...
          alert('Librarian Register Not Succesfull,User Id might already exist,Email Id or Password must be Invalid')
        }
      });
      ...
      this.http.post('http://localhost:8080/librarian/addStudent', registerForm.value).subscribe(response => {
        ...
        if (response) {
          ...
          alert('Student Register Not Succesfull,User Id might already exist,Email Id or Password must be Invalid')
        }
      });
    }
  }
